...
var TouchControls = function() {
    ...
    this.draw = function(ctx) {
        ...
        this.drawButton(ctx, gutterWidth, y, '\u25C0', Game.keys.left);
        this.drawButton(ctx, unitWidth + gutterWidth, y, '\u25B6', Game.keys.right);
        this.drawButton(ctx, 4 * unitWidth + gutterWidth, y, 'A', Game.keys.fire);
        ctx.restore();
    };
    ...
    this.trackTouch = function(e) {
        ...
        for (var i=0; i < e.targetTouches.length; i++) {
            ...
            if (x < unitWidth) Game.keys.left = true;
            if (x > unitWidth && x < 2*unitWidth) Game.keys.right = true;
        }
        if (e.type == 'touchstart' || e.type == 'touchend') {
            for (i=0; i < e.targetTouches.length; i++) {
                touch = e.targetTouches[i];
                x = touch.pageX / Game.canvasMultiplier - Game.canvas.offsetLeft;
                if (x > 4 * unitWidth) Game.keys.fire = (e.type == 'touchstart');
            }
        }
    };
 ...
 * @param level Array of enemy layout data in a level
 * @param behaviors Array of behaviours for each object in the level layout
 * @param winCallback triggererd when all alien objects are cleared from this level
 * @param loseCallback triggererd when all lives lose on this level
 *
 */

var Level = function(level, behaviors, winCallback, loseCallback) {
    this.level = [];    // local copy to play with
    for (var i = 0; i <level.length; i++) {
        this.level.push(Object.create(level[i]));
    }
    this.t = 0;
    this.behaviors = behaviors;
    this.win = winCallback;
    this.lose = loseCallback;
};
...
Level.prototype.step = function(dt) {
    ...
    while ((_cObj = this.level[_i]) && (_cObj[0] < this.t + 2000)) {
        if (this.t > _cObj[1]) {
            _remove.push(_cObj); // remove if past the end
        }
        else if (_cObj[0] < this.t) {
            this.board.add(new Enemy(this.behaviors[_cObj[3]], _cObj[4])); // add sprite
    ...
    for (var i = 0; i < _remove.length; i++) {
        _i = this.level.indexOf(_remove[i]);
        if(_i != -1) this.level.splice(_i, 1);
    }

    // Check if level won or lost
    if (this.level.length === 0 && this.board.count[Game.types.OBJECT_ENEMY] === 0) {
